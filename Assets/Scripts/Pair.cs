...
public class Pair : MonoBehaviour
{
	...
	internal void SendLetter(string p)
	{
		...
		{ 
			...
			{
				Game.Me.GetComponent<Game>().PapaGameObject.GetComponent<AudioSource>().PlayOneShot(Camera.main.GetComponent<Game>().PapaSound,1);
				...
				Game.Me.PapaGameObject.GetComponent<Animator>().SetTrigger("Success");
